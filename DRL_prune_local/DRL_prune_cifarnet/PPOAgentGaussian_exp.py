...
class GaussianActorCritic(nn.Module):
    def __init__(self, rnn_hidden_size, prunable_layers_n_channels, sigma=0.05):
        ...
        # Since each layer has different number of channels, need to craete 1 encoder for each layer
        self.encoders = []
    ...
    def predict_action(self, state, budget, layer_index, hidden, with_noise=True, is_train_gagent=None):
        ...
        #Actor (predict mean and sd)
        #Mean
        action_mu = self.decoder_mu(x)
        action_mu = action_mu  + budget #shift the center of the dist around the budget range -> sample action around budget
        #SD
        ...
        #Critic (predict the value of trainable param)
