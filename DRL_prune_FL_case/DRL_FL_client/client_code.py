...
import torch
import os
import DRL_client_cifarnet.DRL_cifarnet_client as drl_cfn
import websockets
import asyncio
import time
import sys
...
if model_type == 'cfn':
    os.makedirs('./prune_cfn',exist_ok=True)
    model_save_path = './prune_cfn/cifarnet_recv.pth'
    train_save_path = './prune_cfn/cfn_trained.pth'
# elif model_type == 'mv1':
...
def main():
    ...
    if model_type == 'cfn':
        ...
        train_dataloader = drl_cfn.load_dataset()
        model = drl_cfn.get_net()
        ...
        model,optimizer = drl_cfn.load_model(model, model_save_path)
        train_start_time = time.time()
        print('Training the model locally......')
        drl_cfn.train_basenet(model, train_dataloader,optimizer,train_save_path)
        ...
        with open('./prune_cfn/Report.txt', 'a') as outs:
...
async def connect_send():
    ...
    async with websockets.connect(uri) as websocket:
        if model_type == 'cfn':
            await websocket.send('aggregate_drl_cifarnet')
        # elif model_type == 'mv1':
