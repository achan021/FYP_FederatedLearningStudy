...
import DRL_client_cifarnet.utils.operator
...
class PruningInference(nn.Module):
    ...
    def forward(self, x, budget, static_budget, is_train_gagent, is_train_base, with_noise=None):
        assert self.actor is not None #check that self.actor is not none
        ...
        rl_info = []
        static_info = []
        assert self.layer_index is None #check that self.layer_index is not none
        assert self.hidden is None #check that self.hidden is not none
        ...
        with torch.autograd.set_grad_enabled(is_train_base):
            #partial is used to pre-fill the arguments for the calling function prune later
            x = self.base_net(
                    x, 
    ...
    def prune(self, x, states, actions, rl_info, with_noise, is_train_gagent, static_info):
        ...
        mask = DRL_client_cifarnet.utils.operator.convert_n10p1_to_01(torch.sign(prune_channel_begin - mask), zero_to_p1=True)
