...
import DRL_client_cifarnet.hook_cifarnet_static_dynamic_agg as hook_cifarnet
from DRL_client_cifarnet.BudgetPruning_static_dynamic_exp import PruningInference
from DRL_client_cifarnet.PPOAgentGaussian_exp import GaussianActorCritic
from DRL_client_cifarnet.PPOAgentGaussian_static_exp import GaussianActorCritic_static
...
def train_basenet(pruning_net, train_loader, optimizer,train_save_path):
    epochs = 5
    ...
    for e in range(epochs):
        ...
        for i, (images, labels) in tqdm(enumerate(train_loader)):
            ...
            sample_budget = torch.ones([images.shape[0], 1]) * arg['budget']
            static_budget = torch.ones([images.shape[0], 1]) * arg['static_budget']
            ...
            total_accuracy += batch_correct/len(pred_output)
        ...
        train_accuracy_per_epoch.append(total_accuracy/len(train_loader))
        train_losses_per_epoch.append(total_loss/len(train_loader))     
        ...
        print('Train BaseNet - Epoch {} complete. loss: {:.6g}. Accuracy : {}'.format(
            e+1,
    ...
    with open('./prune_cfn/training_time.txt', 'w') as os:
